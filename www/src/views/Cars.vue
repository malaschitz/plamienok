...
<script>
    export default {
        ...
        data: () => ({
          headers: [{text: 'Skratka', value: 'Name', align: 'left'},{text: 'Popis', value: 'Popis'},
        ...
        methods: {
          ...
          deleteItem: function(item) {
            ...
            if (confirm(conf)) {
              this.$axios.delete("/r/api/car/" + item.ID).then(response => {
          ...
          save: function() {
            ...
            this.$axios.post("/r/api/car",this.editedItem).then(response => {
              if (response.status == 202) {
                this.$store.commit("alert","Chyba: " + response.data.Error);
              } else { //OK
          ...
          readData: function() {
            this.$axios.get('/r/api/cars').then(response => {

  ...
  <v-container fluid>
    ...
    >
      <template v-slot:items="props">
        ...
        <td class="text-left">
          <v-chip v-if="props.item.IsPlanned" color="primary" text-color="white" small>Plánovaná</v-chip>

          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-chip v-if="props.item.IsZdravotna" color="info" text-color="white" small>Zdr</v-chip>
            </template>
            <span>Zdravotná</span>
          </v-tooltip>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-chip v-if="props.item.IsSprevadzanie" color="info" text-color="white" small>Spr</v-chip>
            </template>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-chip v-if="props.item.IsSocial" color="info" text-color="white" small v-on="on">Soc</v-chip>
            </template>
            <span>Sociálna</span>
          </v-tooltip>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-chip v-if="props.item.IsPoUmrti" color="info" text-color="white" small>Umr</v-chip>
            </template>
            <span>Po úmrtí</span>
          </v-tooltip>
        </td>
        <td class="text-left text-no-wrap">{{ props.item.Popis }}</td>
        <td
          class="text-left text-no-wrap text-truncate"
          style="max-width: 200px;"
        >{{ props.item.PopisDetail }}</td>
        <td>
          <v-icon class="mr-2" @click="editVisit(props.item)">edit</v-icon>
        </td>
      </template>
    </v-data-table>
  </v-container>
